              ...
              <div className="flex flex-row gap-9 py-4 text-[#9a9a9a] ">
                ...
                <div className='flex flex-col justify-center gap-3 lg:md:w-1/4'>
                    ...
                    <div className='flex flex-col rounded-[25px] text-[14px] font-[500] gap-3 px-6 py-4 text-center justify-center bg-white' style={{boxShadow:'0px 10px 30px 0px rgba(0, 0, 0, 0.05)'}}>
                        ...
                        <p className='text-[#555555] '>Reaching us is easy! Let us know your queries.
                        ...
                        <div className='flex flex-row gap-4 text-[#555555] text-left'><TiLocation size={22} className='text-[#C4C4C4]'/>A47, Lorem ipsum dummy, State, Country-01001</div>
                    </div>
                    ...
                    <div className='flex flex-col rounded-[25px] text-[14px] gap-3 px-12 py-4 text-center text-[#555555] justify-center bg-white' style={{boxShadow:'0px 10px 30px 0px rgba(0, 0, 0, 0.05)'}}>
                        ...
                        <hr/>
                        <div className='flex flex-col'>
                          <div> <h3 className='font-[600] text-[14px]'>{emojie}</h3></div>
                         <div className="flex flex-row items-center justify-evenly gap-4 py-3 cursor-pointer">
                        <FaRegFrown size={25}  onClick={()=>handleEmojie('Dissatisfied')}  className={` rounded-[15px] ${emojie === 'Dissatisfied'? 'text-[#C31A7F] ring-8 ring-[#f9e2ef] ':''}`}/>
                        <FaRegMeh size={25} onClick={()=>handleEmojie('Neutral')}  className={` rounded-[15px] ${emojie === 'Neutral'? 'text-[#C31A7F] ring-8 ring-[#f9e2ef] ':''}`}/>
                        <FaRegSmile size={25} onClick={()=>handleEmojie('Satisfied')}  className={` rounded-[15px] ${emojie === 'Satisfied'? 'text-[#C31A7F] ring-8 ring-[#f9e2ef] ':''}`}/>
                        <FaRegLaughBeam size={25} onClick={()=>handleEmojie('Excellent')} className={` rounded-[15px] ${emojie === 'Excellent'? 'text-[#C31A7F] ring-8 ring-[#f9e2ef] ':''}`}/>
                        </div>
                        {emojie && (
                          <div className='flex flex-col w-full pt-2'>
                            <p className='font-[500] self-start px-2 pb-1'>Where can we improve?</p>
                            <textarea placeholder="Type here..." className='bg-[#e5e5e5] rounded-[15px] p-2 outline-none' height={20}/>
                            <div className='justify-center items-center flex p-2'>
                            <button className=' bg-[#EFC319] text-white rounded-[10px] w-fit py-2 px-6'>Submit</button>
                            </div>
                          </div>
                        )}
